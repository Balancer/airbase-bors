<?
	function lbr_module_online_map($db_name = 'l2jdb')
	{
		...
		if($cache->get('LBR:DB',"$db_name-online"))
			return $cache->last();
		...
		foreach($db->get_array("SELECT char_name, x, y FROM characters WHERE online > 0") as $row)
		{
			...
			$map .=	"<div style=\"position:absolute;top:{$m_y}px;left:{$m_x}px\"><img src=\"/l2jz/modules/map/i/points/monster.gif\" title=\"{$row['char_name']}\"></div>";
		}
