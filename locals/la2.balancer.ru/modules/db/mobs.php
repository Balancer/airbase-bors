<?
	...
	function module_local_db_mobs()
	{
		...
		if(@$_GET['exists'])
		{
			...
			if(@$_GET['exists']=='yes')
				$where .= " AND s.npc_templateid IS NOT NULL";
			else
		...
		foreach($db->get_array("SELECT DISTINCT n.id, n.level, n.name $query LIMIT $start, $limit;", false) as $row)
		{
			...
			$drop = $db->get_array("SELECT d.itemId as item_id, d.sweep, d.chance, d.min, d.max, p.name, gchance FROM `droplist` d LEFT JOIN prices p ON (d.itemId = p.item_id) WHERE `mobId` = $mob_id ORDER BY d.chance*(d.min+d.max)*p.price DESC;");
