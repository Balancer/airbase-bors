...
if (($cur_forum['post_topics'] == '' && $pun_user['g_post_topics'] == '1') || $cur_forum['post_topics'] == '1' || $is_admmod)
	$post_link = "\t\t<p class=\"postlink conr\"><a href=\"{$pun_config['root_uri']}/post.php?fid=$id\">".$lang_forum['Post topic'].'</a></p>'."\n";
else
	$post_link = '';
...
if ($pun_user['is_guest'] || $pun_config['o_show_dot'] == '0')
{
	...
	$sql = 'SELECT id, poster, subject, description, posted, last_post,
		...
		'.(($cur_forum['sort_by'] == '1') ? 'posted' : 'GREATEST(`last_post`, `sort_time`)').' DESC
...
if ($db->num_rows($result))
{
	while ($cur_topic = $db->fetch_assoc($result))
	{
		...
		if ($cur_topic['moved_to'] == null)
			$last_post = "<a href=\"{$pun_config['root_uri']}/viewtopic.php?pid={$cur_topic['last_post_id']}#p{$cur_topic['last_post_id']}\">".format_time($cur_topic['last_post']).'</a> <span class="byuser">'.$lang_common['by'].'&nbsp;'.pun_htmlspecialchars($cur_topic['last_poster']).'</span>';
		else
			$last_post = '&nbsp;';
		...
		if ($cur_topic['moved_to'] != 0)
			$subject = $lang_forum['Moved'].": <a href=\"{$pun_config['root_uri']}/viewtopic.php?id={$cur_topic['moved_to']}\">".pun_htmlspecialchars($cur_topic['subject']).'</a> <span class="byuser">'.$lang_common['by'].'&nbsp;'.pun_htmlspecialchars($cur_topic['poster']).'</span>';
		else if ($cur_topic['closed'] == '0')
			$subject = "<a href=\"{$pun_config['root_uri']}/viewtopic.php?id={$cur_topic['id']}\">".pun_htmlspecialchars($cur_topic['subject']).'</a> <span class="byuser">'.$lang_common['by'].'&nbsp;'.pun_htmlspecialchars($cur_topic['poster']).'</span>';
		else
		{
			$subject = "<a href=\"{$pun_config['root_uri']}/viewtopic.php?id={$cur_topic['id']}\">".pun_htmlspecialchars($cur_topic['subject']).'</a> <span class="byuser">'.$lang_common['by'].'&nbsp;'.pun_htmlspecialchars($cur_topic['poster']).'</span>';
		...
		if (!$pun_user['is_guest'] 
			&& (@$visits[$cur_topic['id']] > 0 || $cur_topic['last_post'] > $pun_user['last_visit'])
			&& $cur_topic['last_post'] > $visits[$cur_topic['id']]
			&& $cur_topic['moved_to'] == null)
		{
			...
			$subject_new_posts = "<span class=\"newtext\">[&nbsp;<a href=\"{$pun_config['root_uri']}/viewtopic.php?id={$cur_topic['id']}&amp;action=new\" title=\"".$lang_common['New posts info'].'">'.$lang_common['New posts'].'</a>&nbsp;]</span>';
		}
		else
		...
		if ($num_pages_topic > 1)
			$subject_multipage = '[ '.paginate($num_pages_topic, -1, "{$pun_config['root_uri']}/viewtopic.php?id={$cur_topic['id']}").' ]';
		else
		...
		$description = &$cur_topic['description'];
		if($description)
...
<div class="linksb">
	<div class="inbox">
		<p class="pagelink conl"><?php echo $paging_links ?></p>
<?php echo $post_link ?>
		<ul><li><a href="<?echo $pun_config['root_uri'];?>/index.php"><?php echo $lang_common['Index'] ?></a>&nbsp;</li><li>&raquo;&nbsp;<?php echo pun_htmlspecialchars($cur_forum['forum_name']) ?></li></ul>
		<div class="clearer"></div>
	</div>
</div>
<?php
