...
$result = $db->query('SELECT c.id AS cid, c.cat_name, f.id AS fid, f.parent, f.forum_name, f.disp_position FROM '.$db->prefix.'categories AS c INNER JOIN '.$db->prefix.'forums AS f ON c.id=f.cat_id ORDER BY c.disp_position, c.id, f.disp_position')
...
?>
								<tr>
									...
									<td>
										Position&nbsp;&nbsp;<input type="text" name="position[<?php echo $cur_forum['fid'] ?>]" size="4" maxlength="4" value="<?php echo $cur_forum['disp_position'] ?>" tabindex="<?php echo $tabindex_count++ ?>" />
										Parent&nbsp;&nbsp;<input type="text" name="parent[<?php echo $cur_forum['fid'] ?>]" size="3" maxlength="3" value="<?php echo $cur_forum['parent'] ?>" tabindex="<?php echo $tabindex_count++ ?>" />
									&nbsp;&nbsp;<strong><?php echo $cur_forum['fid'].":&nbsp;".pun_htmlspecialchars($cur_forum['forum_name']) ?></strong></td>
								</tr>
<?php
