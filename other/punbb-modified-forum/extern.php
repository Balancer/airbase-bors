...
else if ($_GET['action'] == 'stats')
{
	...
	$result = $db->query('SELECT id, username FROM '.$db->prefix.'users WHERE last_post > 0 ORDER BY registered DESC LIMIT 1') or error('Unable to fetch newest registered user', __FILE__, __LINE__, $db->error());
