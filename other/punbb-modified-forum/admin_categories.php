...
$result = $db->query('SELECT id, cat_name, disp_position, parent, base_uri FROM '.$db->prefix.'categories ORDER BY disp_position') or error('Unable to fetch category list', __FILE__, __LINE__, $db->error());
$num_cats = $db->num_rows($result);
...
?>
		<tr><td><?php echo $cat_id?>:&nbsp;<input type="text" name="cat_name[<?php echo $i;?>]" value="<?php echo pun_htmlspecialchars($cat_name);/*"*/?>" size="35" maxlength="80" /></td>
