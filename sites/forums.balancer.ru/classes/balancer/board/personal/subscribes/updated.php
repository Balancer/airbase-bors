...
class balancer_board_personal_subscribes_updated extends balancer_board_page
{
	function title() { return ec('Обновлённия тем, на которые Вы подписаны'); }
	...
	function body_data()
	{
		...
		$topics = bors_find_all('balancer_board_topic', array(
			'inner_join' => array(
				'balancer_board_users_subscription ON (balancer_board_users_subscription.topic_id = balancer_board_topic.id)',
				'topic_visits ON (topic_visits.topic_id = balancer_board_topic.id)',
			),
		...
		$counts = bors_find_all('balancer_board_topic', array(
			...
			'inner_join' => array(
				"balancer_board_users_subscription ON (balancer_board_users_subscription.topic_id = balancer_board_topic.id AND balancer_board_users_subscription.user_id=$me_id)",
				"balancer_board_topics_visit ON (balancer_board_topic.id = balancer_board_topics_visit.target_object_id AND target_class_id = 2 AND balancer_board_topics_visit.user_id = $me_id)",
