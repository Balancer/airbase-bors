...
<div class="alert alert-info text-center" style="padding: 0"><div style="font-size: 1.5rem"><a href="http://forums.balancer.ru/money/" style="color: {if $owner->money()>0}green{elseif $owner->money()<0}red{/if}">☼{$owner->money()}</a></div><div style="font-size: 0,5rem">Ваш баланс</div></div>
