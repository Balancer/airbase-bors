<?
    ...
    { 
        ...
        $txt=preg_replace("/^\s*===== (.+) =====\s*$/me","'<h6>'.lcml(\"$1\").'</h6>'",$txt);
        $txt=preg_replace("/^\s*==== (.+) ====\s*$/me","'<h5>'.lcml(\"$1\").'</h5>'",$txt);
        $txt=preg_replace("/^\s*=== (.+) ===\s*$/me","'<h4>'.lcml(\"$1\").'</h4>'",$txt);
        $txt=preg_replace("/^\s*== (.+) ==\s*$/me","'<h3>'.lcml(\"$1\").'</h3>'",$txt);
